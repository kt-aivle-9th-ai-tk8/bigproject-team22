...
(
    ...
    -- 같은 단지 안에서 현장코드는 유일해야 한다(중복 적재/오등록 방지).
    CONSTRAINT uq_turbines_farm_code UNIQUE (wind_farm_id, turbine_code),
...
(
    ...
    -- 한 터빈 안에서 블레이드 태그(A/B/C)는 유일해야 한다.
    CONSTRAINT uq_blades_turbine_tag UNIQUE (turbine_id, blade_tag),
